...
const auctionSlice = createSlice({
  ...
  reducers: {
    ...
    getWinningAuctionsRequest(state, action) {
      state.loading = true;
      state.winningAuctions = [];
    },
    getWinningAuctionsSuccess(state, action) {
      state.loading = false;
      state.winningAuctions = action.payload;
    },
    getWinningAuctionsFailed(state, action) {
      state.loading = false;
      state.winningAuctions = [];
    },
    resetSlice(state, action) {
      ...
      state.winningAuctions = state.winningAuctions;
    },
  },
});
...
export const getWinningAuctions = () => async (dispatch) => {
  dispatch(auctionSlice.actions.getWinningAuctionsRequest());
  try {
    const response = await axios.get(
      "http://localhost:5000/api/v1/auctionitem/winning-auctions",
      { withCredentials: true }
    );
    dispatch(
      auctionSlice.actions.getWinningAuctionsSuccess(response.data.winningAuctions)
    );
    dispatch(auctionSlice.actions.resetSlice());
  } catch (error) {
    dispatch(auctionSlice.actions.getWinningAuctionsFailed());
    console.error(error);
    dispatch(auctionSlice.actions.resetSlice());
  }
};
