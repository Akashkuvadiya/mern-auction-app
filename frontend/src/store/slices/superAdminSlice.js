...
const superAdminSlice = createSlice({
  ...
  reducers: {
    requestForMonthlyRevenue(state, action) {
      ...
      state.bidderMonthlyPayments = [];
      state.auctioneerMonthlyPayments = [];
      state.monthlyTransactionAmounts = [];
      state.monthlyTransactionCounts = [];
      state.monthlyPaymentProofAmounts = [];
      state.monthlyPaymentProofCounts = [];
    },
    successForMonthlyRevenue(state, action) {
      state.loading = false;
      state.monthlyRevenue = action.payload.totalMonthlyRevenue;
      state.bidderMonthlyPayments = action.payload.bidderMonthlyPayments;
      state.auctioneerMonthlyPayments = action.payload.auctioneerMonthlyPayments;
      state.monthlyTransactionAmounts = action.payload.monthlyTransactionAmounts || [];
      state.monthlyTransactionCounts = action.payload.monthlyTransactionCounts || [];
      state.monthlyPaymentProofAmounts = action.payload.monthlyPaymentProofAmounts || [];
      state.monthlyPaymentProofCounts = action.payload.monthlyPaymentProofCounts || [];
    },
    ...
    requestForUpdateUserWallet(state, action) {
      state.loading = true;
    },
    successForUpdateUserWallet(state, action) {
      state.loading = false;
    },
    failureForUpdateUserWallet(state, action) {
      state.loading = false;
    },
    clearAllErrors(state, action) {
      state.loading = false;
      state.monthlyRevenue = state.monthlyRevenue;
      state.bidderMonthlyPayments = state.bidderMonthlyPayments;
      state.auctioneerMonthlyPayments = state.auctioneerMonthlyPayments;
      state.monthlyTransactionAmounts = state.monthlyTransactionAmounts;
      state.monthlyTransactionCounts = state.monthlyTransactionCounts;
      state.monthlyPaymentProofAmounts = state.monthlyPaymentProofAmounts;
      state.monthlyPaymentProofCounts = state.monthlyPaymentProofCounts;
...
export const getMonthlyRevenue = () => async (dispatch) => {
  ...
  try {
    ...
    if (!response.data || !response.data.success) {
      console.error("Monthly revenue API response is invalid:", response.data);
      dispatch(superAdminSlice.actions.failedForMonthlyRevenue());
      return;
    }
    
    dispatch(
      superAdminSlice.actions.successForMonthlyRevenue(response.data)
    );
  } catch (error) {
...
export const getAllUsers = () => async (dispatch) => {
  ...
  try {
    ...
    if (!response.data || !response.data.success) {
      console.error("User stats API response is invalid:", response.data);
...
export const updateUserWallet = (userId, data) => async (dispatch) => {
  dispatch(superAdminSlice.actions.requestForUpdateUserWallet());
  try {
    const response = await axios.put(
      `http://localhost:5000/api/v1/superadmin/user/wallet/${userId}`,
      data,
      { withCredentials: true, headers: { "Content-Type": "application/json" } }
    );
    dispatch(superAdminSlice.actions.successForUpdateUserWallet());
    toast.success(response.data.message);
    dispatch(superAdminSlice.actions.clearAllErrors());
  } catch (error) {
    dispatch(superAdminSlice.actions.failureForUpdateUserWallet());
