...
export const verifyCommissionCron = () => {
    cron.schedule("*/1 * * * *", async () => {
        ...
        const approveProofs = await PaymentProof.find({ status: "Approved" });
        for (const proof of approveProofs) {
            try {
                const user = await User.findById(proof.userId);
                let updatedUserData = {};  // Corrected variable name
                if (user) {
                    ...
                    const message = `Dear ${user.userName},\n\nWe are pleased to inform you that your recent payment has been successfully verified and settled.
                         ...
                         Your account has been updated, and you can now proceed with your activities on our platform 
